...
5. Suspends backend deploys and sets `JOINED_ARCHIVE_WORKER_URL`, `JOINED_ARCHIVE_CAPABILITY_KEY`, and `JOINED_ARCHIVE_WORKER_TOKEN` through separate `PUT .../env-vars/KEY` calls. After every PUT, it prints keys only and confirms those three plus `STORAGE_CRED_KEY`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_ACCOUNT_ID`, and `DATABASE_URL` remain present.
