...
echo "starting capture-server: worker_id=${WORKER_ID} server_id=${CAPTURE_SERVER_ID} capture_shared_capacity=${CAPTURE_SERVER_CAPTURE_SHARED_CAPACITY} heartbeat_sec=${CAPTURE_SERVER_HEARTBEAT_SEC} lease_sec=${CAPTURE_SERVER_LEASE_SEC} refresh_sec=${CAPTURE_TICK_SEC}"
if [[ -n "${CAPTURE_SERVER_EXECUTION_CLASSES:-}" ]]; then
