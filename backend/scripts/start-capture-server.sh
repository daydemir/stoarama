...
export CAPTURE_SEGMENT_TARGET_FPS="${CAPTURE_SEGMENT_TARGET_FPS:-10}"
...
cmd=(
  ...
  --segment-target-fps "${CAPTURE_SEGMENT_TARGET_FPS}"
...
echo "starting capture-server: worker_id=${WORKER_ID} server_id=${CAPTURE_SERVER_ID} capture_shared_capacity=${CAPTURE_SERVER_CAPTURE_SHARED_CAPACITY} segment_target_fps=${CAPTURE_SEGMENT_TARGET_FPS} heartbeat_sec=${CAPTURE_SERVER_HEARTBEAT_SEC} lease_sec=${CAPTURE_SERVER_LEASE_SEC} refresh_sec=${CAPTURE_TICK_SEC}"
if [[ -n "${CAPTURE_SERVER_EXECUTION_CLASSES:-}" ]]; then
