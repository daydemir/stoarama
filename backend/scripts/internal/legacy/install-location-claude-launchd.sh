...
LOG_BASENAME="${LOG_BASENAME:-${LABEL_SWEEPER}}"
DRY_RUN="${DRY_RUN:-0}"
...
if ! command -v "${CLAUDE_BIN:-claude}" >/dev/null 2>&1; then
  echo "install error: Claude binary not found: ${CLAUDE_BIN:-claude}" >&2
  exit 1
fi

mkdir -p "${AGENTS_DIR}" "${LOG_DIR}"

cat >"${SWEEPER_PLIST}" <<EOF_PLIST
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>-lc</string>
      <string>cd "${ROOT_DIR}" &amp;&amp; SI_ENV_FILE="${ENV_FILE}" backend/scripts/start-location-claude-sweeper.sh</string>
    </array>
...
echo "log files:"
echo "  ${LOG_DIR}/${LOG_BASENAME}.out.log"
echo "  ${LOG_DIR}/${LOG_BASENAME}.err.log"
