...
func detectCompletedWindowHealth(ctx context.Context, pool *pgxpool.Pool) ([]healthIncident, error) {
	rows, err := pool.Query(ctx, `
		...
		FROM latest l
		LEFT JOIN recording_clips c ON c.recording_id=l.id AND c.recording_job_id=l.job_id
...
func detectContinuousWindowEndedEarly(ctx context.Context, pool *pgxpool.Pool) []healthIncident {
	rows, err := pool.Query(ctx, `
		...
		WHERE j.kind='continuous_window' AND j.status IN ('done','error') AND j.window_end_at>now()
		  ...
		  AND NOT EXISTS (SELECT 1 FROM recording_clips cl WHERE cl.recording_id=j.recording_id AND cl.recording_job_id=j.id AND cl.clip_start_at >= j.fire_at)
	`)
	if err != nil {
