...
func main() {
	...
	case "recording-joined":
		runRecordingJoined(ctx, cfg, os.Args[2:])
...
func usage() {
  ...
  stoaramactl inference cleanup-unboxed [--pipeline-id P --mode requeue|delete --dry-run]
	  ...
	  stoaramactl nodes relay-groups list [--backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups create --name NAME [--max-streams 8 --bandwidth-mbps N --backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups update --id N [--name NAME --max-streams N --bandwidth-mbps N --backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups assign --id N --node-id N [--backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups unassign --node-id N [--backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups delete --id N [--backend-api-url URL --api-token TOKEN]
	  ...
	  stoaramactl recording-joined freeze-tier1-checkpointed --connection-id N --batch-id ID --source-endpoint URL --qualification-run-id N [--generation N]
	  ...
	  stoaramactl recording-joined admission (status|pause|resume|refill-one|drain) [--batch-id ID] [--timeout-sec 7200]
