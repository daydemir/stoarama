...
func usage() {
  ...
  stoaramactl inference cleanup-unboxed [--pipeline-id P --mode requeue|delete --dry-run]
	  ...
	  stoaramactl nodes relay-groups create --name NAME [--max-streams 8 --backend-api-url URL --api-token TOKEN]
	  stoaramactl nodes relay-groups update --id N [--name NAME --max-streams N --backend-api-url URL --api-token TOKEN]
