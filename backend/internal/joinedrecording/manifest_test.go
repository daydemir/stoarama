...
func TestBatchIndexV1Golden(t *testing.T) {
	...
	_, canonical, _, err := BuildBatchIndex(index)
	if err != nil {
		t.Fatal(err)
	}
...
func testBatchIndex(t *testing.T) BatchIndex {
	...
	for day := 0; day < 14; day++ {
		...
		ledgerRelative, ledgerObject, _ := CanonicalAllocationLedgerPaths(batchID, 377, localDate)
		sourceCount, sourceBytes := 0, int64(0)
		if day == 0 {
			sourceCount, sourceBytes = 1, 10
		}
		ledgers = append(ledgers, AllocationLedgerRef{ArtifactID: int64(90 + day), RecordingID: 377, LocalDate: localDate, QualificationSHA: strings.Repeat("f", 64), SourceClaimSHA256: strings.Repeat("1", 64), RelativePath: ledgerRelative, ObjectKey: ledgerObject, SizeBytes: 2000, SHA256: strings.Repeat("2", 64), LedgerSHA256: strings.Repeat("d", 64), SourceCount: sourceCount, SourceBytes: sourceBytes, ScheduledHourIDs: hourIDs})
	}
	...
	metadata := testRequest([]SourceClip{testSource(1, time.Date(2026, time.May, 4, 8, 0, 0, 0, time.UTC))}).Metadata
	cutoff := time.Date(2026, time.August, 21, 6, 59, 7, 0, time.UTC)
	index := BatchIndex{SchemaVersion: 1, PolicyVersion: PlanPolicyVersion, AllocationSchemaVersion: 1, HourManifestSchemaVersion: 1, BatchID: batchID, Generation: 1, FrozenAt: cutoff, FrozenDenominatorSHA256: strings.Repeat("a", 64), RecordingIDs: []int64{377}, RecordingIDSHA256: recordingIDsSHA([]int64{377}), FrozenRecordings: []FrozenRecording{{RecordingID: 377, PriorityOrdinal: 1, EligibilityTier: "good+", EligibilityCutoff: cutoff, CompletedAt: cutoff.Add(-time.Hour), Timezone: "UTC", FolderName: "01_Europe_Italy_Bevagna_Piazza_Silvestri", NamingMetadata: metadata}}, MediaTool: mediaTool, ExpectedLedgerCount: 14, ScheduledHourCount: 168, SourceClipCount: 1, SourceBytes: 10, FinalMediaCount: 1, AllocationLedgers: ledgers, Hours: hours}
