...
func TestRecordingCampaignTracksAuditAndProtection(t *testing.T) {
	...
	for _, q := range []string{
		`CREATE TABLE accounts(id bigint primary key)`, `CREATE TABLE users(id bigint primary key)`, `CREATE TABLE streams(id bigint primary key)`,
	...
	if _, err = c.Exec(ctx, `INSERT INTO accounts VALUES(1); INSERT INTO users VALUES(2); INSERT INTO streams VALUES(3); INSERT INTO recordings VALUES(4,1,3)`); err != nil {
		t.Fatal(err)
	}
	var track int64
	if err = c.QueryRow(ctx, `INSERT INTO recording_campaign_tracks(account_id,campaign_key,label,deadline_at,target_count,grade_floor,created_by_user_id) VALUES(1,'delivery30','Delivery 30',now()+interval '7 days',30,'GOOD',2) RETURNING id`).Scan(&track); err != nil {
		t.Fatal(err)
	}
	if _, err = c.Exec(ctx, `INSERT INTO recording_campaign_roster_entries(track_id,recording_id,stream_id,scene_identity_sha256,role,rank,status,reason_codes,effective_at,decision_at,updated_by_user_id) VALUES($1,4,3,repeat('a',64),'primary',1,'protect',ARRAY['current_stable'],now(),now(),2)`, track); err != nil {
		t.Fatal(err)
	}
