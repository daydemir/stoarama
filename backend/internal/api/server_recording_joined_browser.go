...
<div>{{range .Folders}}<div class="entry"><a class="folder" href="{{.Path}}"><span class="folder-icon" aria-hidden="true"></span><span><span class="name">{{.Name}}</span><span class="meta">{{.Count}} file{{if ne .Count 1}}s{{end}}</span></span></a>{{if .ArchivePath}}<div class="actions"><a class="button" href="{{.ArchivePath}}">Download ZIP</a></div>{{end}}</div>{{end}}{{range .Files}}<div class="entry"><div class="file-main"><span class="type {{.TypeClass}}">{{.Type}}</span><div><div class="name">{{.Name}}</div><div class="meta">{{.LocalDate}} · {{.Size}}</div></div></div><div class="actions"><a class="button" href="{{.DownloadPath}}?disposition=inline" target="_blank" rel="noopener noreferrer">View</a><a class="button" href="{{.DownloadPath}}?disposition=attachment">Download</a></div></div>{{end}}{{if and (not .Folders) (not .Files)}}<div class="empty">No joined files are published in this folder yet.</div>{{end}}</div></main></body></html>`))
...
func (s *Server) handleAccountRecordingJoinedFolder(w http.ResponseWriter, r *http.Request) {
	...
	archiveEnabled := s.joinedArchiveConfigured()
	folders, files, found := joinedFolderEntries(result.FolderPath, result.Files, activePath, archiveEnabled)
	if !found {
	...
	if err := joinedFolderTemplate.Execute(w, joinedFolderPage{
		...
		ArchivePath: joinedFolderArchivePath(result.FolderPath, activePath, archiveEnabled),
	}); err != nil {
		return
	}
}
...
func joinedFolderEntries(root string, source []recordingJoinedFile, active []string, archiveEnabled bool) ([]joinedFolderEntry, []joinedFolderFile, bool) {
	...
	for _, file := range source {
		...
		if len(parts) > len(active)+1 {
			...
			if index, exists := folderIndex[name]; exists {
				...
				folderPath := append(append([]string{}, active...), name)
				folders = append(folders, joinedFolderEntry{
					Name: name, Count: 1, Path: joinedFolderPath(root, folderPath),
					ArchivePath: joinedFolderArchivePath(root, folderPath, archiveEnabled),
				})
			}
			continue
		}
