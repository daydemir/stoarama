...
<main class="browser"><nav class="crumbs" aria-label="Joined clip folders">{{range $index, $crumb := .Crumbs}}{{if $index}}<span class="sep" aria-hidden="true">/</span>{{end}}{{if $crumb.Current}}<span aria-current="page">{{$crumb.Name}}</span>{{else}}<a href="{{$crumb.Path}}">{{$crumb.Name}}</a>{{end}}{{end}}{{if .ArchivePath}}<a class="button primary" href="{{.ArchivePath}}">Download folder ZIP</a>{{end}}</nav>
