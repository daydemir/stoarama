...
</style></head><body><nav class="topbar" aria-label="Joined recordings navigation"><a href="{{.BackPath}}">Back to recording</a><a href="{{.RootPath}}">All joined recordings</a></nav><header class="head"><div><h1>{{.RootName}}</h1><p class="note">Published continuous parts and their hour manifests. Separate MP4 files mark a detected gap or incompatible source layout.</p></div><div class="head-actions"><span class="count">{{.MediaTotal}} MP4 · {{.JSONTotal}} JSON</span></div></header>
...
</style></head><body><a class="back" href="{{.BackPath}}">Back to recordings</a><h1>Joined recordings</h1><p class="note">Each folder uses its canonical delivery name. Open a folder to browse its published MP4 files.</p><main class="browser">{{range .Folders}}<div class="entry"><div><a class="folder" href="{{.Path}}"><span class="folder-icon" aria-hidden="true"></span><span class="name">{{.Name}}</span></a><div class="meta">{{.ClipCount}} MP4 · {{.Size}}</div></div></div>{{else}}<div class="empty">No joined recordings are published yet.</div>{{end}}</main></body></html>`))
...
func (s *Server) handleAccountRecordingJoinedFolder(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.recordingJoinedFiles(r.Context(), principal, recordingID,
		[]string{"hour_manifest", "media"}, recordingJoinedBrowserMaxLimit, 0)
	if errors.Is(err, pgx.ErrNoRows) {
		util.WriteError(w, http.StatusNotFound, "recording not found")
		return
	}
	...
	mediaTotal, jsonTotal := 0, 0
	for _, file := range result.Files {
		if file.Kind == "hour_manifest" {
			jsonTotal++
