...
func (s *Server) router() http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Route("/account", func(account chi.Router) {
			...
			account.Put("/nodes/{id}/relay-group/{group_id}", s.handleAccountNodeRelayGroupPut)
			account.Delete("/nodes/{id}/relay-group", s.handleAccountNodeRelayGroupDelete)
			account.Get("/relay-groups", s.handleAccountRelayGroupsList)
			account.Post("/relay-groups", s.handleAccountRelayGroupsCreate)
			account.Patch("/relay-groups/{id}", s.handleAccountRelayGroupPatch)
			account.Delete("/relay-groups/{id}", s.handleAccountRelayGroupDelete)
		...
		api.Group(func(admin chi.Router) {
			...
			admin.Post("/recording/upload-intents/reconcile-expired", s.handleAdminRecordingUploadIntentReconcile)
			...
			admin.Post("/recording/joined/freeze-tier1/dry-run/start", s.handleAdminJoinedFreezeTier1DryRunStart)
			admin.Post("/recording/joined/freeze-tier1/dry-run/step", s.handleAdminJoinedFreezeTier1DryRunStep)
			admin.Get("/recording/joined/freeze-tier1/dry-run/status", s.handleAdminJoinedFreezeTier1DryRunStatus)
			admin.Post("/recording/joined/freeze-tier1/dry-run/invalidate", s.handleAdminJoinedFreezeTier1DryRunInvalidate)
			...
			admin.Post("/recording/joined/batches/final-freeze/validation/start", s.handleAdminJoinedFinalValidationStart)
			admin.Post("/recording/joined/batches/final-freeze/validation/step", s.handleAdminJoinedFinalValidationStep)
			admin.Get("/recording/joined/batches/final-freeze/validation/status", s.handleAdminJoinedFinalValidationStatus)
		...
		api.Group(func(rec chi.Router) {
			...
			rec.Post("/recording/presentation-probes/{taskId}/complete", s.handleRecordingPresentationV2Complete)
			rec.Post("/recording/presentation-probes/{taskId}/retry", s.handleRecordingPresentationV2Retry)
		...
		api.Group(func(joinedOperator chi.Router) {
			...
			joinedOperator.Put("/recording/joined/retry-grant", s.handleJoinedExactRetryGrant)
