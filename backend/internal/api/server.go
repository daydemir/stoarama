...
func (s *Server) router() http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(op chi.Router) {
			...
			op.Get("/streams", s.handleStreamsList)
			op.Post("/source-candidates", s.handleSourceCandidatesUpsert)
			op.Get("/source-candidates", s.handleSourceCandidatesList)
			op.Post("/source-candidates/{id}/review", s.handleSourceCandidateReview)
			op.Post("/source-candidates/{id}/import", s.handleSourceCandidateImport)
			op.Post("/source-candidates/{id}/runs", s.handleSourceCandidateRunCreate)
			op.Post("/recording/streams/{id}/assign", s.handleRecordingStreamAssign)
			op.Post("/recording/streams/{id}/unassign", s.handleRecordingStreamUnassign)
			op.Get("/recording/assignments", s.handleRecordingAssignmentsList)
			op.Get("/recording/assignments/audit", s.handleRecordingAssignmentsAudit)
			op.Post("/recording/servers/heartbeat", s.handleRecordingServerHeartbeat)
			op.Post("/recording/servers/stopped", s.handleRecordingServerStopped)
			op.Post("/youtube-relay/sources/heartbeat", s.handleYouTubeRelaySourceHeartbeat)
			op.Post("/youtube-relay/sources/stopped", s.handleYouTubeRelaySourceStopped)
			op.Get("/youtube-relay/routes", s.handleYouTubeRelayRoutesList)
			op.Post("/youtube-relay/routes/{stream_id}/status", s.handleYouTubeRelayRouteStatus)
			op.Patch("/streams/{id}/capture", s.handleStreamsCapturePatch)

			op.Post("/pipelines/sync", s.handlePipelinesSync)
			op.Get("/pipelines", s.handlePipelinesList)

			op.Get("/capture/schema", s.handleCaptureSchema)
			op.Get("/capture/streams", s.handleCaptureStreams)
			op.Get("/capture/runtime", s.handleCaptureRuntime)
			op.Post("/capture/runtime/stopped", s.handleCaptureRuntimeStopped)
			op.Post("/capture/worker-heartbeat", s.handleCaptureWorkerHeartbeat)
			op.Post("/capture/worker-stopped", s.handleCaptureWorkerStopped)
			op.Post("/recording/process/heartbeat", s.handleRecordingProcessHeartbeat)
			op.Post("/recording/process/stopped", s.handleRecordingProcessStopped)
			op.Post("/processing/worker-heartbeat", s.handleProcessingWorkerHeartbeat)
			op.Post("/processing/worker-stopped", s.handleProcessingWorkerStopped)
			op.Post("/capture/ingest", s.handleCaptureIngest)
			op.Post("/capture/mark-unsupported", s.handleCaptureMarkUnsupported)
			op.Post("/inference/claims", s.handleInferenceClaims)
			op.Post("/inference/claims/abandon", s.handleInferenceClaimsAbandon)
			op.Post("/inference/commit", s.handleInferenceCommit)
			op.Post("/inference/fail", s.handleInferenceFail)
			...
			op.Get("/dashboard/countries", s.handleDashboardCountries)
			op.Get("/dashboard/cities", s.handleDashboardCities)
			op.Get("/dashboard/sources", s.handleDashboardSources)
			op.Get("/dashboard/youtube-channels", s.handleDashboardYouTubeChannels)
			op.Get("/dashboard/tags", s.handleDashboardTags)
			op.Post("/dashboard/streams/image-urls", s.handleDashboardStreamImageURLs)
			op.Get("/dashboard/inference", s.handleDashboardInference)
			op.Post("/dashboard/inference/cleanup-unboxed", s.handleDashboardInferenceCleanupUnboxed)
			op.Get("/dashboard/recording/settings", s.handleDashboardRecordingSettingsGet)
			op.Put("/dashboard/recording/settings", s.handleDashboardRecordingSettingsPut)
			op.Get("/dashboard/recording/capacity", s.handleDashboardRecordingCapacityList)
			op.Get("/dashboard/recording/server-capacity", s.handleDashboardRecordingServerCapacity)
			op.Get("/dashboard/recording/summary", s.handleDashboardRecordingSummary)
			op.Get("/dashboard/servers", s.handleDashboardServers)
			op.Get("/dashboard/pipelines/overview", s.handleDashboardPipelinesOverview)
			op.Get("/dashboard/queue-health", s.handleDashboardQueueHealth)
			op.Get("/dashboard/streams/{id}", s.handleDashboardStreamDetail)
			op.Get("/dashboard/streams/{id}/pipelines", s.handleDashboardStreamPipelinesList)
			op.Put("/dashboard/streams/{id}/pipelines/{pipeline_id}", s.handleDashboardStreamPipelineUpsert)
			op.Get("/dashboard/streams/{id}/timeline", s.handleDashboardStreamTimeline)
			op.Get("/dashboard/streams/{id}/coverage", s.handleDashboardStreamCoverage)
			op.Get("/dashboard/streams/{id}/capture-samples", s.handleDashboardStreamCaptureSamples)
			op.Get("/dashboard/streams/{id}/detections", s.handleDashboardStreamDetections)
			op.Get("/dashboard/streams/{id}/recording", s.handleDashboardStreamRecording)
			op.Get("/dashboard/streams/{id}/frame-manifest", s.handleDashboardStreamFrameManifest)
			op.Post("/dashboard/streams/{id}/frame-exports", s.handleDashboardStreamFrameExportCreate)
			op.Get("/dashboard/streams/{id}/frame-exports/{export_id}", s.handleDashboardStreamFrameExportGet)
		})
	})
