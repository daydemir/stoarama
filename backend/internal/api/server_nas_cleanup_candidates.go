...
func (s *Server) handleAdminNASCleanupCandidateCreate(w http.ResponseWriter, r *http.Request) {
	...
	runID := uuid.New()
	_, err = tx.Exec(r.Context(), `INSERT INTO nas_cleanup_candidate_runs(id,account_id,connection_id,recording_ids,inventory_generation,inventory_digest,inventory_started_at,inventory_completed_at,state,item_count,total_bytes,unknown_count,nas_rehash_required,canonical_digest,created_by_user_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'queued',$9,$10,$9,true,$11,$12)`, runID, req.AccountID, req.ConnectionID, ids, generation, digest, started, completed, len(items), actualBytes, canonical, p.UserID)
	if err != nil {
	...
	for ordinal, x := range items {
		...
		err = tx.QueryRow(r.Context(), `INSERT INTO r2_content_verifications(storage_destination_id,endpoint_snapshot,bucket,object_key,expected_size_bytes,expected_sha256) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(storage_destination_id,endpoint_snapshot,bucket,object_key,expected_size_bytes,expected_sha256) DO UPDATE SET updated_at=r2_content_verifications.updated_at RETURNING id`, x.DestinationID, x.Endpoint, x.Bucket, x.ObjectKey, x.Size, x.SHA).Scan(&verificationID)
		if err != nil {
			util.WriteError(w, 500, err.Error())
			return
		}
	...
	util.WriteJSON(w, 201, map[string]any{"id": runID, "state": "queued", "canonical_digest": canonical, "item_count": len(items), "total_bytes": actualBytes, "unknown_count": len(items), "nas_rehash_required": true, "qualification_only": true, "reclaim_bytes": 0})
}
...
func (s *Server) handleAdminNASCleanupCandidateGet(w http.ResponseWriter, r *http.Request) {
	...
	var out struct {
		...
		AccountID, ConnectionID                     int64
		RecordingIDs                                []int64
		Generation, Digest, State, Canonical, Error string
		ItemCount, TotalBytes, Unknown              int64
		Rehash                                      bool
		Created                                     time.Time
		Finished                                    *time.Time
	}
	err = s.pool.QueryRow(r.Context(), `SELECT id,account_id,connection_id,recording_ids,inventory_generation,inventory_digest,state,canonical_digest,error_code,item_count,total_bytes,unknown_count,nas_rehash_required,created_at,finished_at FROM nas_cleanup_candidate_runs WHERE id=$1`, id).Scan(&out.ID, &out.AccountID, &out.ConnectionID, &out.RecordingIDs, &out.Generation, &out.Digest, &out.State, &out.Canonical, &out.Error, &out.ItemCount, &out.TotalBytes, &out.Unknown, &out.Rehash, &out.Created, &out.Finished)
	if errors.Is(err, pgx.ErrNoRows) {
