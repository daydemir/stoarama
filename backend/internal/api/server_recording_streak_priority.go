...
func (s *Server) handleAccountRecordingStreakPriority(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.pool.Query(r.Context(), `
		WITH eligible AS (
		  ...
		  SELECT r.id,r.name,r.status,j.fire_at,j.window_end_at,count(*)::int job_count,min(j.id) job_id
		  FROM eligible r JOIN recording_jobs j ON j.recording_id=r.id AND j.kind='continuous_window' AND j.window_end_at<=now()
		  WHERE j.fire_at>=r.start_at AND (r.end_at IS NULL OR j.window_end_at<=r.end_at)
		    AND (j.fire_at AT TIME ZONE r.cron_timezone)::time='08:00:00'::time
		    AND (j.window_end_at AT TIME ZONE r.cron_timezone)::time='20:00:00'::time
		    AND (r.active_weekdays & (1 << (extract(isodow FROM j.fire_at AT TIME ZONE r.cron_timezone)::int-1)))<>0
		  GROUP BY r.id,r.name,r.status,j.fire_at,j.window_end_at
		), recent AS (
		...
		WHERE q.rn<=30 ORDER BY q.id,q.window_end_at DESC`, p.AccountID)
	if err != nil {
