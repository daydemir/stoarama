...
func (s *Server) handleAccountRecordingCampaignTracks(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.pool.Query(r.Context(), `
		SELECT t.campaign_key,t.label,t.deadline_at,t.target_count,t.grade_floor,t.required_consecutive_windows,t.state,
		 e.recording_id,e.stream_id,e.scene_identity_sha256,e.role,e.rank,e.status,e.reason_codes,e.decision_at,
		 r.name,r.status,
		 j.fire_at,j.window_end_at,j.status,
		 COALESCE((SELECT count(*) FROM recording_clips c WHERE c.recording_job_id=j.id),0),
		...
		LEFT JOIN LATERAL (SELECT count(*) FILTER(WHERE coverage_pct>=95 AND largest_gap_seconds<=900 AND gap_over_5m_count<=1 AND gap_over_30s_count<=6 AND overlap_count=0)::int good_windows,
		...
		WHERE t.account_id=$1 AND t.state IN ('draft','active')
	...
	for rows.Next() {
		...
		var deadline, decision time.Time
		...
		if err := rows.Scan(&key, &label, &deadline, &target, &floor, &required, &trackState, &recordingID, &streamID, &scene, &role, &rank, &rosterState, &reasons, &decision, &recName, &recState, &fire, &end, &jobState, &clips, &goodWindows, &totalWindows, &latestCoverage, &latestMedia, &latestIngest); err != nil {
		...
		if fire != nil {
			...
			default:
				checkpoint = "capturing_verified_first_3"
			}
		}
		items = append(items, map[string]any{"campaign_key": key, "label": label, "deadline_at": deadline, "target_count": target, "grade_floor": floor, "required_consecutive_windows": required, "track_state": trackState, "recording_id": recordingID, "stream_id": streamID, "scene_identity_sha256": scene, "role": role, "rank": rank, "roster_status": rosterState, "reason_codes": reasons, "decision_at": decision, "recording_name": recName, "recording_status": recState, "next_window_start_at": fire, "next_window_end_at": end, "job_status": jobState, "current_job_clip_count": clips, "checkpoint": checkpoint, "health_metric_good_candidate_windows": goodWindows, "health_metric_windows": totalWindows, "latest_coverage_pct": latestCoverage, "latest_media_at": latestMedia, "latest_ingest_at": latestIngest})
	}
