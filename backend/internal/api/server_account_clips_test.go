...
func testAccountClipsPool(t *testing.T) (*pgxpool.Pool, func()) {
	...
	for _, stmt := range []string{
		...
		`CREATE TABLE connections (
			...
			client_boot_id TEXT NOT NULL DEFAULT '',
			client_phase TEXT NOT NULL DEFAULT '',
			client_previous_exit TEXT NOT NULL DEFAULT '',
			client_last_success_at TIMESTAMPTZ,
			client_last_error TEXT NOT NULL DEFAULT '',
			client_last_error_at TIMESTAMPTZ,
			last_outage_class TEXT NOT NULL DEFAULT '',
			last_outage_started_at TIMESTAMPTZ,
			last_outage_recovered_at TIMESTAMPTZ,
			last_outage_failure_count INTEGER NOT NULL DEFAULT 0,
			nas_batch_completed_at TIMESTAMPTZ,
			nas_batch_clips INTEGER NOT NULL DEFAULT 0,
			nas_batch_bytes BIGINT NOT NULL DEFAULT 0,
		...
		`CREATE TABLE nas_inventory_files (
			...
			file_ctime_ns BIGINT,
			file_inode BIGINT,
			file_device BIGINT,
			sidecar_relative_path TEXT,
			sidecar_size_bytes BIGINT,
			sidecar_sha256 TEXT,
