...
) RETURNS TABLE(evidence_id BIGINT,evidence_sha256 TEXT,stream_id BIGINT,authority_code TEXT) LANGUAGE plpgsql SET search_path FROM CURRENT AS $$
...
BEGIN
  ...
  PERFORM 1 FROM stream_source_revisions rev WHERE rev.stream_id=presented.stream_id ORDER BY rev.id FOR SHARE;
  SELECT (SELECT max(rev.id) FROM stream_source_revisions rev WHERE rev.stream_id=s.id),
  ...
  stream_id:=presented.stream_id;
  authority_code:=presented.authority_code;
  RETURN NEXT;
END $$;
...
BEGIN
  ...
  EXECUTE format('GRANT SELECT ON TABLE %I.protected_campaign_recordings TO %I',install_schema,authority_role);
