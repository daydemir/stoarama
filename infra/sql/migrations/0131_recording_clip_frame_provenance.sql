...
ALTER TABLE frames ADD COLUMN source_recording_destination_id BIGINT REFERENCES storage_destinations(id) ON DELETE RESTRICT;
ALTER TABLE frames ADD COLUMN source_recording_endpoint TEXT;
ALTER TABLE frames ADD COLUMN source_recording_bucket TEXT;
ALTER TABLE frames ADD COLUMN source_recording_object_key TEXT;
ALTER TABLE frames ADD COLUMN source_recording_size_bytes BIGINT;
...
ALTER TABLE frames ADD CONSTRAINT frames_recording_clip_provenance_coherent CHECK (
  (source_recording_clip_id IS NULL AND source_recording_clip_sha256 IS NULL AND source_recording_clip_etag IS NULL AND source_recording_clip_version_id IS NULL
   AND source_recording_destination_id IS NULL AND source_recording_endpoint IS NULL AND source_recording_bucket IS NULL
   AND source_recording_object_key IS NULL AND source_recording_size_bytes IS NULL)
  OR
  (source_recording_clip_id IS NOT NULL
   ...
   AND source_recording_size_bytes BETWEEN 1 AND 134217728
...
BEGIN
  IF ROW(OLD.stream_id,OLD.captured_at,OLD.raw_media_object_id,OLD.source_kind,
         ...
         OLD.source_recording_destination_id,OLD.source_recording_endpoint,OLD.source_recording_bucket,
         OLD.source_recording_object_key,OLD.source_recording_size_bytes)
     IS DISTINCT FROM
     ROW(NEW.stream_id,NEW.captured_at,NEW.raw_media_object_id,NEW.source_kind,
         ...
         NEW.source_recording_clip_etag,NEW.source_recording_clip_version_id,
         NEW.source_recording_destination_id,NEW.source_recording_endpoint,NEW.source_recording_bucket,
         NEW.source_recording_object_key,NEW.source_recording_size_bytes) THEN
